...
export const evaluateVoiceTurn = async (payload: EvaluationPayload): Promise<EvaluationResponse> => {
...
export const getTTS = async (text: string): Promise<Blob> => {
  const response = await api.post('/infer/tts', { text }, { responseType: 'blob' });
  return response.data;
};