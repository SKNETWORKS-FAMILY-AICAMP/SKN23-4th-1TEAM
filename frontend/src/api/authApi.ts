...
export const authApi = {
  ...
  updateProfileImage: async (formData: FormData) => {
    const response = await axiosClient.post('/api/auth/profile-image', formData, {
  ...
  checkEmail: async (email: string) => {
    const response = await axiosClient.get(`/api/auth/check-email?email=${email}`);
    return response.data;
  },

  sendSignupEmail: async (data: { email: string; auth_code: string }) => {
    const response = await axiosClient.post('/api/auth/send-signup-email', data);
    return response.data;
  },
