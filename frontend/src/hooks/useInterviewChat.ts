...
export const useInterviewChat = () => {
      ...
      if (sessionId) {
        try {
          ...
          if (sessionData.items && sessionData.items.length > 0) {
            ...
            sessionData.items.forEach((detail: any) => {
              if (detail.question) {
                restoredMessages.push({ role: 'assistant', content: detail.question });
                lastAiQuestion = detail.question;
              }
              if (detail.answer) {
                restoredMessages.push({ role: 'user', content: detail.answer, score: detail.score });
              }
            });

            setMessages(restoredMessages);
            pendingQuestionRef.current = lastAiQuestion;
            currentQIdxRef.current = sessionData.items.filter((d: any) => !d.is_followup).length;
            return;
          }
        } catch (error) {
          console.error(error);
        ...
        try {
          ...
          if (res.data?.session_id) {
            localStorage.setItem('current_session_id', res.data.session_id.toString());
          }
        } catch (error) {
          console.error(error);
        }
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (sessionId) {
        await axiosClient.post('/api/interview/details', {
          session_id: sessionId,
      ...
      if (reply_text.includes('[INTERVIEW_END]')) {
        ...
        if (sessionId) {
          axiosClient.post('/api/infer/end', { session_id: sessionId })
            .catch(err => console.error(err));
        }
      }
