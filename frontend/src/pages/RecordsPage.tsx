...
const SessionDetailModal = ({ 
  ...
  const avgScore = scoredDetails.length > 0 
    ? scoredDetails.reduce((sum, d) => sum + (d.score || 0), 0) / scoredDetails.length 
    : 0;
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <div className="modal-header">
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <button 
              ...
              style={{ display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: '#2563eb', color: 'white', border: 'none', padding: '8px 14px', borderRadius: '6px', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}
            >
        ...
        {loading ? (
          ...
          <p className="modal-message empty">이 세션에 저장된 상세 기록이 없습니다.</p>
        ) : (
            ...
            <div className="detail-list">
              {details.map((d, idx) => (
                <div key={idx} className="detail-card-item">
                  {Boolean(d.is_followup) && <span className="tag-followup">꼬리질문</span>}
                  
                  <div className="q-section">
                    <div className="q-header">
                      Q{d.turn_index + 1}. 면접관 질문
                      {d.score !== null && (
                        <span className={`score ${d.score >= 7 ? 'green' : (d.score >= 5 ? 'orange' : 'red')}`}>
        ...
        <button className="btn-close-full" onClick={onClose}>닫기</button>
      </div>
    </div>
  );
};
...
export const RecordsPage = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      setLoading(true);
      const response = await axiosClient.get(`/api/infer/sessions?user_id=${user.id}`);
  ...
  const handleDelete = async (sessionId: number) => {
    if (!window.confirm(`세션 #${sessionId} 기록을 정말 삭제하시겠습니까?\n삭제 후에는 복구할 수 없습니다.`)) return;
    try {
      ...
      <main className="records-main">
        <div className="page-header flex-header">
          <div>
            ...
            <p className="hero-subtitle">진행 완료된 결과 확인 및 기록을 관리할 수 있습니다.</p>
          </div>
          <button className="primary-action-btn sm" onClick={() => navigate(ROUTES.INTERVIEW)}>
            새 모의면접 시작
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-desc">모의 면접을 시작해 첫 번째 기록을 만들어보세요.</p>
            <button className="primary-action-btn" onClick={() => navigate(ROUTES.INTERVIEW)}>
              모의면접 하러 가기
            </button>
          </div>
        ) : (
            ...
            {records.map((record) => (
              <div key={record.id} className="record-card">
                <div className="card-header">
                  ...
                  <button className="btn-delete" onClick={() => handleDelete(record.id)}>삭제</button>
                </div>
                
                <div className="card-body">
                  <div className="score-section">
                    <span className="score-label">
                      {record.status === 'COMPLETED' ? '종합 평가 점수' : '진행 상태'}
                    </span>
                    <div className="score-value-wrapper">
                      <span className={`score-value ${getScoreColorClass(record.total_score)}`}>
                        {record.status === 'COMPLETED' 
                          ? (record.total_score !== null ? `${(record.total_score <= 10 ? record.total_score * 10 : record.total_score).toFixed(1)}점 / 100` : '채점 중')
                          : <span className="status-in-progress" style={{color: '#ef4444'}}>중단됨</span>
                        }
                      </span>
                    </div>
                  </div>
                ...
                <div className="card-footer">
                  <span className="date-text">일시: {formatDate(record.started_at)}</span>
                  {record.status === 'COMPLETED' ? (
                    <button 
