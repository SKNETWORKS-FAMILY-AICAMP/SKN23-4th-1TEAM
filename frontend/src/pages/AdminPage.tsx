...
export const AdminPage = () => {
  ...
  const activeUsers = users.filter(u => u.status !== 'withdrawn' && u.status !== 'dormant').length;
    ...
    <div className="admin-layout">
      <header className="admin-header">
        <div className="logo-area" onClick={() => navigate(ROUTES.HOME)}>
          <span className="logo-text">AI<span>WORK</span></span>
          <span className="logo-sub">관리자 대시보드</span>
        </div>
        <div className="user-area">
          <span>{user?.name || 'admin'} 님 </span>
          <button className='btn-admin-logout' onClick={handleLogoutClick}>로그아웃</button>
        </div>
      </header>
            ...
            <>
              ...
              <div className="stats-grid">
                <div className="stat-card">
                  ...
                  <div className="stat-value">{users.length} <span className="unit">명</span></div>
                </div>
                <div className="stat-card">
                  <span className="stat-label">활성 회원</span>
                  <div className="stat-value active">{activeUsers} <span className="unit">명</span></div>
                </div>
                <div className="stat-card">
                  <span className="stat-label">PREMIUM 회원</span>
                  <div className="stat-value plus">{plusUsers} <span className="unit">명</span></div>
                </div>
              </div>
              ...
              <div className="admin-section">
                <div className="section-header">
                  <h3>전체 사용자 목록</h3>
                  <button className="btn-refresh" onClick={fetchUsers}>새로고침</button>
                </div>
                      ...
                      <tbody>
                        {users.map(u => (
                          <tr key={u.id}>
                            ...
                            <td>{u.email}</td>
                            <td>{u.name}</td>
                            <td className="masked">{u.password ? '********' : 'None'}</td>
                            <td>{u.role}</td>
                            <td>{u.tier}</td>
              ...
              <div className="admin-section control-panel">
                ...
                <div className="control-grid">
                  <div className="control-box">
                    <h4>회원 등급 변경</h4>
                    <select value={selectedTierUser} onChange={e => setSelectedTierUser(e.target.value)}>
                      {users.map(u => <option key={`t-${u.id}`} value={u.id}>{u.id}: {u.name || u.email}</option>)}
                    </select>
                    <select value={selectedTier} onChange={e => setSelectedTier(e.target.value)}>
                      <option value="normal">normal</option>
                      <option value="premium">premium</option>
                    </select>
                    <button className="btn-primary full" onClick={() => handleUpdate('tier')}>
                      등급 변경 적용
                    </button>
                  </div>
                  ...
                  <div className="control-box">
                    <h4>계정 상태 관리</h4>
                    <select value={selectedStatusUser} onChange={e => setSelectedStatusUser(e.target.value)}>
                      {users.map(u => <option key={`s-${u.id}`} value={u.id}>{u.id}: {u.name || u.email}</option>)}
                    </select>
                    ...
                    <button className="btn-primary full" onClick={() => handleUpdate('status')}>
                      상태 변경 적용
                    </button>
                  </div>
                </div>
                ...
                <div className="admin-role-box">
                  ...
                  <p className="current-admins">
                    현재 관리자: <strong>{users.filter(u => u.role === 'admin').map(u => u.name || u.email).join(', ')}</strong>
                  </p>
                  <div className="role-actions">
                    <select value={selectedRoleUser} onChange={e => setSelectedRoleUser(e.target.value)}>
                      {users.map(u => <option key={`r-${u.id}`} value={u.id}>{u.id}: {u.name || u.email} [{u.role}]</option>)}
                    </select>
                    <button className="btn-primary" onClick={() => handleUpdate('grant_admin')}>관리자 부여</button>
                    <button className="btn-outline" onClick={() => handleUpdate('revoke_admin')}>권한 해제</button>
                  </div>
                </div>
              </div>
            </>
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <p style={{ margin: '0 0 24px 0', fontSize: '14px', color: '#64748b' }}>
              관리자 세션이 종료되며<br/>로그인 화면으로 이동합니다.
            </p>
            <div style={{ display: 'flex', gap: '12px' }}>
              <button 
                onClick={() => setShowLogoutModal(false)}
                style={{ flex: 1, padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#ffffff', color: '#475569', fontWeight: '700', cursor: 'pointer' }}
              >
                취소
              </button>
              <button 
                onClick={executeLogout}
                style={{ flex: 1, padding: '12px', borderRadius: '12px', border: 'none', background: '#0176f7', color: '#ffffff', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(1, 118, 247, 0.3)' }}
              >
                확인
              </button>
            </div>
