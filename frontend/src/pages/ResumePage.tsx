...
export const ResumePage = () => {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!uploadFile) { setUploadError('이력서 파일을 업로드해주세요.'); return; }
    if (!user?.id) return;
    ...
    try {
      ...
      if (!text.trim()) {
        setUploadError('텍스트를 추출할 수 없습니다. 다른 형식의 파일을 사용해주세요.');
        return;
      }
    ...
    <div className="resume-page-layout">
      ...
      <main className="resume-main">
        <div className="page-header">
          ...
          <p className="hero-subtitle">분석된 이력서를 관리하고 모의면접을 진행하세요.</p>
        </div>
          ...
          <div className="list-container">
            {filteredResumes.length === 0 ? (
              ...
              filteredResumes.map((r) => (
                <div key={r.id} className="list-item" onClick={() => setSelectedResume(r)}>
                  <div className="item-info">
                    <span className="job-badge">{r.job_role}</span>
                    <h3 className="item-title" title={r.title}>{r.title}</h3>
                  </div>
                  
                  <div className="item-meta">
                    <div className="meta-group">
                      <span className="meta-label">등록일</span>
                      <span className="meta-value">{formatDate(r.created_at)}</span>
                    </div>
                    <div className="meta-group">
                      <span className="meta-label">적합도</span>
                      <span className="meta-value highlight">{r.analysis_result?.match_rate ?? 0}%</span>
                    </div>
                  </div>

                  <div className="item-actions">
                    <button className="btn-view" onClick={(e) => { e.stopPropagation(); setSelectedResume(r); }}>
                      결과 보기
                    </button>
                    <button className="btn-delete" onClick={(e) => handleDelete(r.id, e)}>
                      삭제
                    </button>
                  </div>
                </div>
            ...
            <form onSubmit={handleUploadSubmit} className="modal-body">
              ...
              <div className="form-group">
                ...
                >
                  ...
                  />
                  {uploadFile
                    ? <span className="file-name selected">{uploadFile.name}</span>
                    : <span className="file-name">여기를 클릭하여 파일을 선택하세요 (PDF, TXT)</span>}
                </div>
              </div>

              {uploadError && <div className="error-message">{uploadError}</div>}

              <button className="primary-action-btn full-width" type="submit" disabled={uploading}>
