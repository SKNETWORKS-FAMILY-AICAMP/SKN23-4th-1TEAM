...
import { X, Paperclip, ArrowUp, Database, Save, Download, CheckCircle, AlertCircle, Info, Loader2 } from 'lucide-react';
...
export const GuideChatbot = () => {
  ...
  const [confirmModalPayload, setConfirmModalPayload] = useState<{ title: string, text: string, msgId: string } | null>(null);
  const [isSaving, setIsSaving] = useState(false);
  
  const [savedPayloadIds, setSavedPayloadIds] = useState<Set<string>>(new Set());
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
        showToast('지원하지 않는 형식입니다. PDF 또는 TXT 파일만 올려주세요!', 'warning');
  ...
  const handleQuickReply = (type: string) => {
    ...
    setMessages((prev) => [...prev, { id: Date.now().toString(), type: 'bot', content: aiResponse }]);
  };
  ...
  const handleSend = async () => {
    ...
    setMessages((prev) => [...prev, { id: Date.now().toString(), type: 'user', content: userMessageContent.trim() }]);
    ...
    try {
      ...
      if ((fileToUpload || dbResumeToUse) && wantsAnalysis && !isInterviewIntent) {
        ...
        if (wantsProofread && wantsAnalysis) {
          loadingText = '이력서 분석 및 첨삭을 동시에 진행 중입니다. 시간이 조금 더 걸릴 수 있습니다...';
        } else if (wantsProofread) {
          loadingText = '문서를 분석하고 첨삭을 진행 중입니다...';
        }

        setMessages((prev) => [...prev, { id: loadingId, type: 'bot', content: loadingText }]);
        ...
        if (wantsAnalysis) {
          if (fileToUpload) {
            const ingestRes = await inferApi.ingestResume(fileToUpload, `session_${Date.now()}`);
            if (ingestRes.resume_text) {
              try {
                const analysisRes = await inferApi.analyzeResume(ingestRes.resume_text, "기본 직무");
                ...
                savePayload = { title: fileToUpload.name, text: ingestRes.resume_text };
              } catch (e) {
      ...
      const errorDetail = error.response?.data?.detail || error.message || '알 수 없는 서버 오류 (500)';
      setMessages((prev) => [...prev, { 
        id: Date.now().toString(), 
        type: 'bot', 
  ...
  const executeSaveResume = async () => {
    ...
    try {
      ...
      setSavedPayloadIds(prev => new Set(prev).add(confirmModalPayload.msgId));
      ...
      {confirmModalPayload && (
        <div className="confirm-modal-overlay">
          <div className="confirm-modal-content">
            ...
            <p>
              저장된 이력서는 다음 면접이나 첨삭 시<br />간편하게 다시 불러올 수 있습니다.
            </p>
              ...
              <div key={msg.id} className={`message-row ${msg.type}`}>
                ...
                <div className="message-content">
                  <div className="message-bubble">
                    {msg.content.split('\n').map((line, i) => (
                      <Fragment key={i}>
                        {line.startsWith('**') ? <strong>{line.replace(/\*\*/g, '')}</strong> : line.startsWith('###') ? <h4 style={{margin: '8px 0'}}>{line.replace('###', '')}</h4> : line.startsWith('---') ? <hr style={{ border: 'none', borderTop: '1px dashed #e2e8f0', margin: '12px 0' }} /> : line}
                        <br />
                      </Fragment>
                    ))}
                      ...
                      <div className="bubble-actions-row">
                        {msg.downloadContent && (
                          <button 
                            ...
                            onClick={() => handleDownload(msg.downloadContent!, msg.downloadFilename!)}
                          >
                        ...
                        {msg.saveResumePayload && (
                          <button 
                            ...
                            onClick={() => setConfirmModalPayload({ ...msg.saveResumePayload!, msgId: msg.id })}
