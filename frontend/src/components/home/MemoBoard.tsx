...
export const MemoBoard = () => {
  ...
  if (loading) return <div className="loading-state">메모를 불러오는 중입니다...</div>;

  return (
    <div className="memo-board-container">
      <form onSubmit={handleSubmit} className="memo-form">
        <p><strong>응원의 한마디 남기기</strong></p>
      ...
      <div className="memos-grid">
        {memos.map((memo, idx) => (
          <div 
            key={memo.id || idx} 
            className="memo-card" 
            style={{ background: memo.color, border: `1px solid ${memo.border}`, color: memo.text_color }}
          >
