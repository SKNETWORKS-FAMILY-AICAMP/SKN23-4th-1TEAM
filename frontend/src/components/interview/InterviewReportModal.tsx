...
export const InterviewReportModal = ({ messages, onRestart }: Props) => {
  ...
  const [loadingText, setLoadingText] = useState('AI가 면접 내용을 분석 중입니다...');
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      parsedSections.forEach((_, idx) => { allOpen[idx] = true; });
          ...
          <div ref={pdfRef} style={{ padding: '20px', backgroundColor: '#fff' }}>
            ...
            <div className="score-circle-container">
              ...
              >
                <svg className="progress-ring" viewBox="0 0 160 160">
                  <circle className="progress-ring__bg" cx="80" cy="80" r="74" />
                  <circle 
            ...
            {!experienceText && (
              <div className="warning-banner">
                이력서를 연동하지 않은 자율 면접이므로, 직무 매칭률 평가는 제한될 수 있습니다.
              </div>
            )}
              ...
              {isLoading ? (
                ...
                <div className="accordion-list">
                  {parsedSections.map((section, idx) => (
                    <div key={idx} className={`accordion-item ${openSections[idx] ? 'open' : ''}`}>
        ...
        <div className="modal-fixed-footer">
          <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
            <button 
              ...
              style={{ flex: 1, backgroundColor: '#f8fafc', color: '#475569', border: '1px solid #cbd5e1' }}
