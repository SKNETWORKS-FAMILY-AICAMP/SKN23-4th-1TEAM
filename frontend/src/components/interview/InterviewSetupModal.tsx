...
export const InterviewSetupModal = ({ onClose }: Props) => {
  ...
  const handleStart = async () => {
    const isResumeProvided = resumeType === 'file' 
      ? selectedFile !== null 
      : experienceText.trim() !== '';
    ...
    try {
      if (resumeType === 'file' && selectedFile) {
        ...
        try {
          ...
          alert("백엔드 서버로 이력서를 전송하는데 실패했습니다. 서버 상태를 확인해주세요.");
          setIsUploading(false);
          return;
        }
      ...
      if (finalQuestions.length < questionCount) {
        ...
        for (let i = 0; i < shortfall; i++) {
          finalQuestions.push({ question: `${jobRole} 직무와 관련된 핵심 기술 경험에 대해 설명해주세요. (${i + 1})` });
        }
      }
    ...
    <>
      <div className="modal-overlay">
        <div className="setup-modal">
          <button className="close-btn" onClick={handleCloseClick}><X size={24} /></button>
          
          <div className="modal-header">
            <h2>AI 모의면접 환경 설정</h2>
            <p>지원자님의 역량을 최대한 발휘할 수 있도록 면접 환경을 설정해주세요.</p>
          </div>

          <div className="settings-grid">
            <div className="setting-section">
              <label className="section-label"><MessageSquare size={16} /> 진행 방식</label>
            ...
            <div className="setting-section">
              <label className="section-label"><Mic size={16} /> 면접관 스타일</label>
          ...
          <div className="setting-section full-width">
            <label className="section-label"><Edit3 size={16} /> 세부 면접 설정</label>
            <div className="details-grid">
              <div className="input-field">
                <span>지원 직무</span>
                <select value={jobRole} onChange={(e) => setJobRole(e.target.value)}>
                  <option value="">직무를 선택해주세요</option>
                  <option value="Python 백엔드 개발자">Python 백엔드 개발자</option>
              ...
              <div className="input-field">
                ...
                <select value={difficulty} onChange={(e) => {
                  if(e.target.value === '상') handleProSelect(setDifficulty, '상');
          ...
          <div className="setting-section full-width resume-section">
            <label className="section-label"><FileText size={16} /> 이력서 및 경험 연동</label>
          ...
          >
            {isUploading ? (
              ...
              isFormValid ? '설정 완료 후 면접 시작' : '이력서 첨부 또는 텍스트 입력을 완료해주세요'
            )}
          </button>
        </div>
      </div>
      ...
      {showConfirmClose && (
        <div className="confirm-close-overlay">
          <div className="confirm-close-modal">
            <h3>면접 설정 종료</h3>
            <p>설정 중인 내용이 저장되지 않습니다.<br/>정말 홈으로 돌아가시겠습니까?</p>
            <div className="confirm-actions">
              <button className="cancel-btn" onClick={cancelClose}>계속 설정하기</button>
              <button className="confirm-btn" onClick={confirmClose}>종료하기</button>
            </div>
          </div>
        </div>
      )}
