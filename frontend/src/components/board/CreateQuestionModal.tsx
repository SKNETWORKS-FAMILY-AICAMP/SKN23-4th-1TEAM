...
export const CreateQuestionModal = ({ onClose, onSuccess }: Props) => {
  ...
  const handlePreSubmit = () => {
    ...
    if (rawContent === rejectedContent) {
      showToast(
        "이미 반려된 내용입니다. 텍스트를 수정한 후 다시 시도해주세요.",
        "warning",
      );
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.response?.status === 409) {
        showToast(
          "이미 게시판에 비슷한 내용의 질문이 등록되어 있습니다! 다른 질문을 올려보세요.",
          "warning",
        );
        ...
        if (detail === "INVALID_CONTENT") {
          showToast(
            "면접과 관련 없는 내용이나 장난스러운 글은 등록할 수 없어요.",
            "error",
          );
        } else if (detail === "TECHNICAL_CONTENT") {
          showToast(
            "인성면접에 올바르지 않은 질문입니다. (기술 면접 질문 감지)",
            "error",
          );
        } else {
  ...
  return (
    <div className="board-modal-overlay" onClick={onClose}>
      ...
      <div
        className="board-modal-content create-modal-content"
      ...
      >
        ...
        <div className="modal-body">
          ...
          <p className="guide-text">
            ...
            AI가 내용을 분석하여 기술 질문 및 부적절한 내용을 필터링하고,{" "}
            <strong>깔끔한 면접 질문 형태로 자동 교정</strong>하여 등록해
            드립니다.
          </p>
        ...
        <div className="modal-footer modal-footer-actions">
          ...
          <button
            ...
            disabled={
              isSubmitting ||
              rawContent.trim().length < 5 ||
              rawContent === rejectedContent
            }
          >
            {isSubmitting ? (
