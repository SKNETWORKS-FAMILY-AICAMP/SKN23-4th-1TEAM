...
export const FindPassword = ({ onSwitchMode }: FindPasswordProps) => {
  ...
  const handleResetPassword = async () => {
    if (!newPassword || !pwPattern.test(newPassword) || newPassword !== confirmPassword) {
  ...
  return (
    <div className="find-pw-container">
      ...
      <p className="info-text-top">가입하신 아이디(이메일)을/를 입력해주세요.</p>
      {step === 1 && (
        <div className="step-container">
          ...
          <button className="submit-btn" onClick={handleSendEmail} disabled={isLoading}>이메일 인증</button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="step-container">
          ...
          {error && <div className="status-msg text-error">{error}</div>}
          {success && <div className="status-msg text-success">{success}</div>}
          <button className="submit-btn" onClick={handleVerifyCode}>인증 확인</button>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="step-container">
          ...
          {error && <div className="status-msg text-error">{error}</div>}
          <button className="submit-btn" onClick={handleResetPassword} disabled={isLoading}>변경 완료</button>
        </div>
      )}

      <div className="helper-links">
        <button className="text-btn" onClick={() => onSwitchMode('login')}>로그인으로 돌아가기</button>
      </div>
