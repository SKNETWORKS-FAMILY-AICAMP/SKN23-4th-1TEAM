...
export const SignUpForm = ({ onSwitchMode }: SignUpFormProps) => {
  ...
  const [showTermsModal, setShowTermsModal] = useState<{ title: string; content: string } | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    if (!name || !namePattern.test(name)) return setGlobalError('이름 형식을 확인해주세요.');
    if (!password || !pwPattern.test(password)) return setGlobalError('비밀번호 형식을 확인해주세요.');
    if (password !== confirmPassword) return setGlobalError('비밀번호가 일치하지 않습니다.');
    if (!agreeTerms || !agreePrivacy) return setGlobalError('필수 약관에 모두 동의해주세요.');
  ...
  return (
    <div className="signup-form-container">
      ...
      {isEmailChecked && isEmailAvailable === false && (
        <div className="field-msg text-error">이미 가입되거나 유효하지 않은 아이디입니다.</div>
      )}
      ...
      {!isVerified ? (
        ...
        <button type="button" className="verify-trigger-btn disabled" disabled>인증 완료</button>
      )}
      ...
      <div className="input-group">
        ...
        {name && (namePattern.test(name) ? 
          <div className="field-msg text-success">올바른 이름 형식입니다.</div> : 
          <div className="field-msg text-error">한글과 영어만 입력 가능합니다.</div>
        )}
      </div>
      ...
      <div className="input-group">
        ...
        />
        {password && (pwPattern.test(password) ? 
          <div className="field-msg text-success">안전한 비밀번호입니다.</div> : 
          <div className="field-msg text-error">형식에 맞게 입력해주세요.</div>
        )}
      </div>
      ...
      <div className="input-group">
        ...
        {confirmPassword && (password === confirmPassword ? 
          ...
          <div className="field-msg text-error">비밀번호가 일치하지 않습니다.</div>
        )}
      </div>
      ...
      <div className="terms-box">
        ...
        <div className="term-desc">필수 이용약관 및 개인정보 수집/이용 동의를 포함합니다.</div>
        <hr />
        <div className="term-row">
          <label>
            <input type="checkbox" checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} />
            <span><strong>(필수)</strong> <strong>AIWORK</strong> 이용약관</span>
          </label>
          <button type="button" onClick={() => setShowTermsModal({ title: 'AIWORK 이용약관', content: TERMS_AIWORK })}>보기</button>
        </div>
        <div className="term-row">
          <label>
          ...
          <button type="button" onClick={() => setShowTermsModal({ title: '개인정보 수집/이용 동의', content: TERMS_PRIVACY })}>보기</button>
        </div>
      </div>

      {globalError && <div className="status-msg text-error">{globalError}</div>}
      ...
      <div className="helper-links">
        <button type="button" className="text-btn" onClick={() => onSwitchMode('login')}>이미 계정이 있으신가요? 로그인</button>
      </div>
        ...
        <div className="custom-modal-overlay">
          <div className="custom-modal">
            <div className="modal-header">
              <h3>이메일 인증</h3>
              <button type="button" onClick={() => setShowVerifyModal(false)}><X size={20}/></button>
            </div>
            <div className="modal-body">
              <p>본인 확인을 진행합니다.<br/>가입하실 이메일 주소로 인증번호를 발송합니다.</p>
              <input type="text" value={email} disabled className="disabled-input" />
              <button type="button" className="action-btn" onClick={handleSendCode} disabled={isLoading}>
              ...
              {isCodeSent && (
                <>
                  ...
                  />
                  <button type="button" className="action-btn primary" onClick={handleVerifyCode}>인증 확인</button>
                </>
              )}
            </div>
          </div>
        ...
        <div className="custom-modal-overlay">
          <div className="custom-modal large">
            <div className="modal-header">
              ...
              <button type="button" onClick={() => setShowTermsModal(null)}><X size={20}/></button>
            </div>
            <div className="modal-body terms-content">
              {showTermsModal.content}
            </div>
            <button type="button" className="action-btn primary" style={{ margin: '20px 20px', width: 'calc(100% - 40px)' }} onClick={() => setShowTermsModal(null)}>확인</button>
          </div>
        </div>,
        document.body
      )}
      ...
      {showSuccessModal && createPortal(
        <div className="custom-modal-overlay">
          <div className="custom-modal text-center">
            <div className="icon" style={{ fontSize: '50px', marginBottom: '10px' }}>👾</div>
            <h2 style={{ color: '#0176f7', margin: '0 0 15px 0' }}>환영합니다!</h2>
            <p><b>{name}</b>님,<br/>회원가입이 성공적으로 완료되었습니다.</p>
            <p style={{ fontSize: '13px', color: '#888', marginTop: '20px' }}>3초 후 로그인 페이지로 자동 이동합니다...</p>
          </div>
        </div>,
        document.body
      )}
