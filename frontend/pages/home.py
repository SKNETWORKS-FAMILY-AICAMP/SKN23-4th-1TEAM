    ...
    with right_col:
        ...
        with st.container(border=True):
            ...
            with c1:
                ...
                st.markdown(f'<img src="{avatar_url}" style="width:60px; height:60px; border-radius:50%; object-fit:cover; border: 2px solid #f1f3f5;">', unsafe_allow_html=True)
            with c2:
                tier_badge_class = "profile-badge-plus" if user_tier.lower() == "plus" else "profile-badge-normal"
                st.markdown(f'<p class="user-name-title">{user_name} <span class="{tier_badge_class}">{user_tier.upper()}</span></p>', unsafe_allow_html=True)
                st.markdown(f'<p class="user-email-text">{user_email}</p>', unsafe_allow_html=True)

            st.markdown("<div style='margin: 15px 0 10px 0; border-bottom: 1px solid #f1f3f5;'></div>", unsafe_allow_html=True)
        ...
        # 안내 카드
        with st.container(border=True):
            st.markdown("<p style='font-size:14px; font-weight:700; color:#111; margin-bottom:5px;'>🔗 Github Repository</p>", unsafe_allow_html=True)
            st.markdown("<p style='font-size:13px; color:#888; margin:0;'>SKN23-3rd-1TEAM 프로젝트 주소</p>", unsafe_allow_html=True)

    # 왼쪽 패널 (정보 및 추천 탭)
    with left_col:
        st.markdown(f"<h3 style='font-size:22px; font-weight:700; color:#111; margin-bottom:20px;'>👋 반가워요, {user_name}님! 오늘 준비되셨나요?</h3>", unsafe_allow_html=True)
        ...
        with tab2:
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown('<div class="alert-info">💡 오늘의 기술 블로그 및 아티클이 노출되는 영역입니다.</div>', unsafe_allow_html=True)
            for title, desc in [
                ...
                with st.container(border=True):
                    st.markdown(f"<p style='font-size:15px; font-weight:700; margin-bottom:4px; color:#111;'>{title}</p>", unsafe_allow_html=True)
                    st.markdown(f"<p style='font-size:13px; color:#666; margin:0;'>{desc}</p>", unsafe_allow_html=True)
        ...
        with tab3:
            ...
            for tip, desc in [
                ...
                with st.container(border=True):
                    st.markdown(f"<p style='font-size:15px; font-weight:700; margin-bottom:4px; color:#111;'>💬 {tip}</p>", unsafe_allow_html=True)
                    st.markdown(f"<p style='font-size:13px; color:#666; margin:0;'>{desc}</p>", unsafe_allow_html=True)
