...
**5.엔터프라이즈급 웹 보안 및 3계층 아키텍처(3-Tier Architecture) 구축**
* 프론트엔드(Streamlit)와 백엔드(FastAPI)의 역할을 엄격하게 분리하여 클라이언트 단의 환경변수(.env) 및 민감 정보(DB 비밀번호, AWS 키 등) 노출 원천 차단
* 모든 데이터 및 AI API 접근은 백엔드 라우터를 거치도록 프록시(Proxy) 구조화하여 시스템 확장성과 보안성 동시 확보
