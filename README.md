...
**5. Agent를 활용한 제로 클릭 네비게이션 구현**
* Agent가 사용자의 자연어 발화에서 면접 세팅 파라미터(지원 직무, 난이도, 면접관 페르소나, 이력서 연동 여부) 자동 추출 로직 적용
* Agent 판단에 따라 일반 텍스트 대화(chat)와 화면 전환 명령(navigate)을 분기하여 JSON 형태로 반환하는 통신 표준 수립
* 대시보드 메인 화면의 챗봇 모달에 Agent API 연동
* 백엔드로부터 수신된 네비게이션 신호 감지 시, 추출된 파라미터를 `st.session_state`에 주입하고 `st.switch_page`를 호출하여 화면 강제 전환 구현
* Agent가 주입한 파라미터가 존재할 경우, 수동 설정 모달창을 우회(Bypass)하고 즉시 AI 모의면접을 시작하는 하이패스 컨트롤러 구축
...
### 7. LUI(Language User Interface) 기반 제로 클릭 네비게이션 구현
**1. AI 에이전트 두뇌 구축 (Backend/AI)**

* OpenAI Function Calling 아키텍처를 활용한 의도 파악 및 라우팅 에이전트(`agent.py`) 구현
* 사용자의 자연어 발화에서 면접 세팅 파라미터(지원 직무, 난이도, 면접관 페르소나, 이력서 연동 여부) 자동 추출 로직 적용
* `find_dotenv`를 활용한 동적 환경 변수 로드 및 예외 처리(401 Unauthorized 방어) 로직 고도화

**2. 에이전트 전용 API 엔드포인트 설계 (FastAPI)**

* 에이전트 제어 신호를 프론트엔드로 전달하기 위한 전용 라우터(`routers/agent.py`) 및 Pydantic 스키마 설계
* 에이전트 판단에 따라 일반 텍스트 대화(chat)와 화면 전환 명령(navigate)을 분기하여 JSON 형태로 반환하는 통신 표준 수립

**3. 프론트엔드 제어권 연동 (Streamlit - home.py)**

* 대시보드 메인 화면의 챗봇 모달에 에이전트 API 연동
* 백엔드로부터 수신된 네비게이션 신호 감지 시, 추출된 파라미터를 `st.session_state`에 주입하고 `st.switch_page`를 호출하여 화면 강제 전환 구현

**4. 핵심 비즈니스 로직 모듈화 및 하이패스(High-pass) 구현 (pages/interview.py)**

* 사용자 입력 UI(모달창)와 면접 생성 비즈니스 로직(DB 세션 생성, RAG 벡터 DB 저장, 면접 질문 생성)을 완벽하게 분리하는 리팩토링 진행
* 에이전트가 주입한 파라미터가 존재할 경우, 수동 설정 모달창을 우회(Bypass)하고 즉시 AI 모의면접을 시작하는 하이패스 컨트롤러 구축
...
<img src="https://img.shields.io/badge/Python-0f29d1?style=for-the-badge&logo=python&logoColor=white"> <img src="https://img.shields.io/badge/FastAPI-d93209?style=for-the-badge&logo=fastapi"> <img src="https://img.shields.io/badge/Streamlit-f23f63?style=for-the-badge&logo=streamlit&logoColor=white">
...
<img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"> <img src="https://img.shields.io/badge/ChromaDB-5175c4?style=for-the-badge&logoColor=white"> <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white">
