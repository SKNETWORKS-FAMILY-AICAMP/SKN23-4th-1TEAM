...
- **의도 파악(Intent Analysis) 라우팅:** 메인 대시보드 우측 하단의 가이드 챗봇에 자연어(예: "내 면접 성적 �```plaintext
🦁 SKN23-4th-1TEAM/
...
│   │   ├── agent.py, evaluator.py, graph.py, prompts.py, question_bank.py, state.py, infer_adapter.py 등
│   ├── api/                # FastAPI v1 하위 API 엔드포인트
│   │   └── v1/endpoints/   # API 라우팅 세부 분리
│   ├── chroma_db/          # 로컬 Chroma 벡터 DB 데이터 저장소
...
│   │   ├── base.py, database.py, session.py, schema_patch.py 등
│   ├── django_api/         # Django 앱 (강력한 라우팅 및 보안 뷰 전담)
│   │   ├── urls.py, views.py
│   │   └── view_modules/   # auth, board, interview, users 등 기능 단위 격리
│   ├── models/             # ORM 데이터 모델 (FastAPI 구동 연동)
│   │   ├── refresh_token.py, user.py, loader.py
│   ├── routers/            # FastAPI 통신 모듈 (비동기 처리 최적화)
│   │   ├── admin, agent, attitude, auth, board, home, infer, interview, jobs, resume, social_auth 모듈
│   ├── schemas/            # Pydantic 기반 입력/출력 인터페이스 규격
│   │   ├── agent, attitude, auth, infer, jobs, resume 스키마 모듈
...
│       └── local_inference.py, social_service.py, tavily_service.py, resume_service.py, jobs_service.py 등
│
...
│   ├── utils/              # 웹캠 추론 연동 파이썬 스크립트 등
│   │   └── webcam_box/     # 웹캠 추론 연동 파이썬 모듈
│   └── src/                # 실제 프론트엔드 비즈니스 소스 트리의 심장
│       ├── App.tsx         # 모든 라우트, 스토어를 감싸는 앱의 뿌리
│       ├── main.tsx        # DOM 렌더러
│       ├── api/            # Axios 클라이언트 및 엔드포인트 통신 모듈
│       │   ├── axiosClient.ts, adminApi.ts, authApi.ts, boardApi.ts, homeApi.ts, inferApi.ts, interviewApi.ts, jobsApi.ts, resumeApi.ts 등
...
│       │   ├── authStore.ts (유저 권한), inferStore.ts (면접 상태), guideChatbotStore.ts
...
│
├── package.json            # 프론트, 백엔드 동시 실행 등 루트 스크립트 및 의존성
└── static/                 # 사용자 프로필 이미지 등 전역 정적 자원 (Django)
    └── profiles/           # 유저 업로드 프로필 사진 저장소
```WebRTC 및 OpenAI Realtime API 기반 지연 없는 STT/TTS 양방향 통신
