...
# <div align="center"> **<span style="color: #0162e0;">AI</span>WORK (고도화된 차세대 AI 주도형 면접 SaaS 플랫폼)** </div>

<div align="center">

<img src="frontend/src/assets/rm/사자개홍보.png" width=90%>
...
<img src="frontend/src/assets/rm/huggingface.png" width="250" alt="웹캠 기반 비언어적 Feature 평가">
...
<img src="frontend/src/assets/systemA.png" width="800">
...
├── package.json            # 루트 레벨 패키지 의존성 (워크스페이스 설정)
├── requirements.txt        # Backend 파이썬 서버 구동 필수 라이브러리 목록
├── docs/                   # 프로젝트 산출물 문서 저장소
│   └── [SKN23-4th-1TEAM] 요구사항 정의서.pdf
│   └── [SKN23-4th-1TEAM] 화면설계서.pdf
│   └── [SKN23-4th-1TEAM] 시스템구성도.pdf
│   └── [SKN23-4th-1TEAM] 테스트계획및결과보고서.pdf
│
├── static/                 # Django 정적 파일 서빙 디렉토리
│   └── profiles/           # 사용자 프로필 이미지 저장소
│
├── backend/                # FastAPI & Django 혼합 메인 백엔드 서버 루트
│   ├── .env                # DB 연동, API 키, 서버 보안 관련 루트 환경변수
│   ├── app.py              # FastAPI 메인 프로토콜 마운트 및 실행 진입점
│   ├── import_questions.py # 면접 질문 데이터를 MySQL에 적재
│   ├── create_admin.py     # 관리자 계정 초기 생성 스크립트
│   │
│   ├── ai/                 # LangGraph 및 평가 파이프라인
│   │   ├── agent.py            # LangGraph 에이전트 노드 정의
│   │   ├── evaluator.py        # 답변 평가 로직
│   │   ├── graph.py            # LangGraph 상태 그래프 구성
│   │   ├── infer_adapter.py    # 로컬 추론 모델 어댑터
│   │   ├── personality_prompts.py  # 면접관 성격 유형별 프롬프트
│   │   ├── prompts.py          # 시스템 프롬프트 모음
│   │   ├── question_bank.py    # DB 기반 질문 풀 관리
│   │   ├── state.py            # LangGraph 상태 타입 정의
│   │   ├── test_eval_prompt.py # 평가 프롬프트 단위 테스트 스크립트
│   │   └── test_run_graph.py   # 그래프 실행 통합 테스트 스크립트
│   │
│   ├── api/                # FastAPI 버전 관리 엔드포인트 계층
│   │   └── v1/
│   │       └── endpoints/
│   │           ├── jobs_api.py     # 채용 공고 API 엔드포인트
│   │           └── resume_api.py   # 이력서 API 엔드포인트
│   │
│   ├── chroma_db/          # ChromaDB 벡터 인덱스 실제 저장소
│   │   └── chroma.sqlite3
│   │
...
│   │   ├── base.py
│   │   ├── database.py
│   │   ├── schema_patch.py # DB 스키마 패치 스크립트
│   │   └── session.py
│   │
│   ├── django_api/         # Django 앱 (강력한 라우팅 및 보안 뷰 전담)
│   │   ├── __init__.py
│   │   ├── apps.py         # Django 앱 설정 클래스
│   │   ├── middleware.py   # 커스텀 미들웨어
│   │   ├── startup.py      # 앱 시작 시 초기화 로직
│   │   ├── urls.py
│   │   ├── utils.py        # 뷰 공통 유틸리티 함수
│   │   ├── views.py
│   │   └── view_modules/   # 기능 단위 격리 뷰 모듈
│   │       ├── __init__.py
...
│   │   ├── admin.py
│   │   ├── agent.py        # AI 에이전트 라우터
│   │   ├── attitude.py
│   │   ├── auth.py
│   │   ├── board.py        # 커뮤니티 게시판 라우터
│   │   ├── home.py
│   │   ├── infer.py
│   │   ├── interview.py
│   │   ├── jobs.py
...
│   │
│   └── services/           # 핵심 비즈니스 로직
│       ├── agent_tools_service.py  # LangGraph 에이전트 툴 서비스
│       ├── attitude_metrics_service.py  # 태도 점수 집계 서비스
│       ├── attitude_service.py     # 웹캠 태도 분석 서비스
│       ├── auth_service.py         # JWT/세션 발급
│       ├── hf_landmark_service.py  # 웹캠 시선 및 표정 처리
│       ├── jobs_service.py
│       ├── llm_service.py          # OpenAI 연동 질의응답 처리
│       ├── local_inference.py      # 로컬 모델 추론 서비스
│       ├── personality_service.py  # 면접관 성격 유형 서비스
...
└── frontend/               # React (Vite) + TS 프론트엔드 워크스페이스
    ...
    ├── tsconfig.app.json   # 앱 전용 TS 컴파일 옵션
    ├── tsconfig.node.json  # Vite 노드 환경 TS 옵션
    ...
    └── src/                # 프론트엔드 소스 루트
        ...
        ├── components/     # 재사용 UI 컴포넌트 (TSX + SCSS)
        │   ├── auth/
        │   │   ├── FindPassword.tsx    # 비밀번호 찾기 폼
        │   │   ├── LoginForm.tsx
        │   │   └── SignUpForm.tsx
        │   ├── board/
        ...
        ├── store/
        │   ├── authStore.ts
        │   ├── guideChatbotStore.ts    # 가이드 챗봇 전역 상태 
        │   └── inferStore.ts
        ├── styles/
        │   ├── _variables.scss
        │   └── global.scss
...
| :---: | :---: | :---: |
| <img src="frontend/src/assets/rm/사용법.png" width="60%"> | <img src="frontend/src/assets/TavilySearch.jpg" width="60%"> |<img src="frontend/src/assets/rm/면접점수확인.png" width="60%"> |
...
| :---: | :---: | :---: |
| <img src="frontend/public/images/common/loading.gif" width="150" align="absmiddle"> | <img src="frontend/src/assets/loadingINT.jpg" width="100%"> |<img src="frontend/src/assets/loadingM.jpg" width="100%"> |
