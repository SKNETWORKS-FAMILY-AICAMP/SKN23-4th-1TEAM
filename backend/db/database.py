...
def create_board_question(content: str) -> int:
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT COALESCE(MAX(display_order), 0) + 1 AS next_order FROM board_questions")
            next_order = cur.fetchone()["next_order"]
            
            cur.execute(
                "INSERT INTO board_questions (content, display_order, is_active) VALUES (%s, %s, 1)",
                (content, next_order)
            )
