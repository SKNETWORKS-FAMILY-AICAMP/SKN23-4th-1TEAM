...
def clear_auth_cookies(res: Response) -> None:
    ...
    res.delete_cookie(settings.CSRF_COOKIE_NAME, path="/", domain=settings.COOKIE_DOMAIN)
...
@router.post("/upgrade")
def upgrade_tier(req: Request, db: Session = Depends(get_db)):
    """결제 완료 후 회원 등급을 Premium으로 업데이트하는 API"""
    user = get_current_user(req, db)
    if not user:
        raise HTTPException(status_code=401, detail="로그인이 필요합니다.")

    # 이미 premium인 경우 그냥 반환
